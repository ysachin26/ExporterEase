...
export async function markNotificationAsRead(notificationId: string) {
  ...
  // In this demo we always use the most-recently-created user.
  const user = await User.findOne().sort({ createdAt: -1 })
  if (!user) {
  ...
  const unreadNotificationCount = dashboard.notifications.filter((n: any) => !n.read).length
